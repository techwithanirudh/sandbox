...
export const RemoteFileStorage = {
  async getSandboxPaths(id: string): Promise<string[]> {
    const res = await fetch(`${process.env.STORAGE_WORKER_URL}/api?sandboxId=${id}`, {
  ...
  async getFolder(folderId: string): Promise<string[]> {
    const res = await fetch(`${process.env.STORAGE_WORKER_URL}/api?folderId=${folderId}`, {
      headers: { Authorization: `${process.env.WORKERS_KEY}` }
    })
    const data: R2Files = await res.json()
    return data.objects.map(obj => obj.key)
  ...
  async fetchFileContent(fileId: string): Promise<string> {
    try {
      const fileRes = await fetch(`${process.env.STORAGE_WORKER_URL}/api?fileId=${fileId}`, {
        headers: { Authorization: `${process.env.WORKERS_KEY}` }
      })
  ...
  async renameFile(fileId: string, newFileId: string, data: string): Promise<boolean> {
  ...
  async getProjectSize(id: string): Promise<number> {
    const res = await fetch(`${process.env.STORAGE_WORKER_URL}/api/size?sandboxId=${id}`, {
      headers: { Authorization: `${process.env.WORKERS_KEY}` }
    })
