...
io.on("connection", async (socket) => {
  try {
    ...
    console.log(
      `[index] User ${data.userId} connected to sandbox ${data.sandboxId} as owner? ${data.isOwner}`,
    )
    ...
    if (!data.isOwner && !connections.ownerIsConnected(data.sandboxId)) {
      console.log(
        `[index] Non-owner tried to connect, but owner not present. Disabling access.`,
      )
    ...
    socket.on("disconnect", async () => {
      console.log(
        `[index] User ${data.userId} disconnected from ${data.sandboxId}`,
      )
      ...
      if (data.isOwner && !connections.ownerIsConnected(data.sandboxId)) {
        console.log(
          `[index] Owner disconnected from sandbox ${data.sandboxId}. Cleaning up.`,
        )
